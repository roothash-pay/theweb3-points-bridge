...
contract PoolManagerRHSTest is Test {
    ...
    function setUp() public {
        ...
        TransparentUpgradeableProxy sourcePoolManagerProxy =
            new TransparentUpgradeableProxy(address(poolManagerRHLogic), admin, "");
        ...
        TransparentUpgradeableProxy usdcTokenProxy = new TransparentUpgradeableProxy(address(usdcTokenImple), admin, "");

        usdcToken = USDCToken(address(usdcTokenProxy));
        usdcToken.initialize(ReLayer);
        sourcePoolManager = PoolManagerRootHash(payable(address(sourcePoolManagerProxy)));
    ...
    function _bridge_ERC20(uint256 amount, address from, address to) internal {
        ...
        uint256 tokenUsdtRate = sourcePoolManager.TokenUSDTExchangeRate(address(erc20Token));
    ...
    function _bridge_WrappedERC20(uint256 amount, address from, address to) internal {
        uint256 bridgeAmount = amount * 1 ether;

        uint256 balanceBefore = sourcePoolManager.FundingPoolBalance(address(usdcToken));
        ...
        sourcePoolManager.BridgeInitiateWrappedERC20(
            SOURCE_CHAIN_ID, DEST_CHAIN_ID, to, address(usdcToken), address(usdcToken), bridgeAmount
        );
        vm.stopPrank();

        assertEq(usdcToken.balanceOf(address(sourcePoolManager)), 0, "Contract WrappedERC20 balance should equal 0");
        assertEq(
            sourcePoolManager.FundingPoolBalance(address(usdcToken)), 0, "FundingPoolBalance for ERC20 should equal 0"
        );
        assertEq(
            sourceMessageManager.nextMessageNumber(),
            nextMessageNumberBefore + 1,
    ...
    function test_Finalize_ETH() public {
        ...
        sourcePoolManager.BridgeFinalizeNativeToken(
            DEST_CHAIN_ID, SOURCE_CHAIN_ID, NativeTokenAddress, cat, seek, amount, 0, 1
        );
        vm.stopPrank();
    ...
    function test_Finalize_ERC20() public {
        ...
        sourcePoolManager.BridgeFinalizeERC20(
            DEST_CHAIN_ID, SOURCE_CHAIN_ID, cat, seek, address(erc20Token), address(erc20Token), amount, 0, 1
        );
        vm.stopPrank();

        assertEq(erc20Token.balanceOf(seek), 10 ether, "Receiver should get bridged ERC20 tokens");
    }
    ...
    function test_Finalize_WrappedERC20() public {
        ...
        sourcePoolManager.BridgeFinalizeWrappedERC20(
            DEST_CHAIN_ID, SOURCE_CHAIN_ID, cat, seek, address(usdcToken), address(usdcToken), amount, 0, 1
        );
        vm.stopPrank();

        assertEq(usdcToken.balanceOf(seek), 1 ether, "Receiver should get bridged WrappedERC20 tokens");
    }
